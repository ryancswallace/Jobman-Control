...
# Synchronize a changelog with every reachable stable semantic-version tag.

set -eu

for command in awk cat git grep mktemp rm; do
...
for release_tag in ${stable_tags}; do
  ...
  if [ -n "${previous_tag}" ]; then
    version_url="https://github.com/ryancswallace/jobman-control/compare/${previous_tag}...${release_tag}"
  else
    version_url="https://github.com/ryancswallace/jobman-control/releases/tag/${release_tag}"
  fi

  if grep -F "## [${version}] - " "${changelog_file}" \
    | grep -Eq '^## \[[0-9]+\.[0-9]+\.[0-9]+\] - [0-9]{4}-[0-9]{2}-[0-9]{2}$'; then
    if ! grep -Fqx "[${version}]: ${version_url}" "${changelog_file}"; then
    ...
    awk \
      ...
      /^\[Unreleased\]: / {
        print "[Unreleased]: https://github.com/ryancswallace/jobman-control/compare/" release_tag "...HEAD"
...
if ! grep -Fqx \
  "[Unreleased]: https://github.com/ryancswallace/jobman-control/compare/${latest_tag}...HEAD" \
  "${changelog_file}"; then
