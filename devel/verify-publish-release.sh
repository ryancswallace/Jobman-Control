...
download_dir=$(mktemp -d \
	"${work_root%/}/jobman-control-release-assets.XXXXXXXXXX")
cleanup() {
	rm -rf "$download_dir"
}
...
checksum_name="jobman-control_${version}_checksums.txt"
signature_name="${checksum_name}.sigstore.json"
for required in \
	"$checksum_name" \
	"$signature_name" \
	jobman-control.intoto.jsonl
do
...
printf '%s\n' \
	"$checksum_name" \
	"$signature_name" \
	jobman-control.intoto.jsonl >>"$expected_assets"
...
image=ghcr.io/ryancswallace/jobman-control
